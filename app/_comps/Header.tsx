    ...
    <header className="bg-gray-800">
      ...
      {mobileMenuOpen && (
        <div className="p-4 space-y-2 bg-gray-700 md:hidden">
          ...
          {session?.user?.email ? <div className='flex items-center justify-around p-1 text-sm bg-gray-900 border rounded cursor-pointer w-1/3' onClick={() => signOut()} >LOGOUT<LogOut className='w-4 h-4' /> </div> : <div className='flex items-center p-1 text-sm bg-gray-900 border rounded cursor-pointer' onClick={() => signIn()} >LOGIN < LogIn className='w-4 h-4' /></div>}
          {/* Add more navigation links as needed */}
        </div>
      )}
    </header>
